...
Então importamos os nossos resolvers e definições de tipos, e configuramos os plugins utilizados, como o Neo4jGraphQLAuthJWTPlugin, que permite a autenticação das requisições através de uma token no campo de "Authorization" no header de cada requisição GraphQL. Tal token carrega consigo criptografada um indentifador do usuário que causou sua geração quando fez um login, o \textit{tipo} deste usuário, e uma assinatura que permite verificação de autenticação posterior.
Após a autenticação do token válido numa requisição futura, ainda é necessário uma camada de autorização para termos certeza que o usuário em questão pode executar exatamente aquela requsição GraphQL. Para isso, a @neo4j/graphql disponibiliza de ferramentas de Role Based Access Control, onde definimos quais operações (CREATE, READ, UPDATE, DELETE) são permitidas por quais tipos de usuários em quais \textit{tipos} de dados. Opcionalmente, é possível a criação de regras genéricas autorização através de expressões regulares.
